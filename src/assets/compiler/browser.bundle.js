...
var Parser = class {
  ...
  parse_task_declaration() {
    ...
    const params = args.map((arg) => {
      if (arg.kind != "Identifier") {
        throw new Error(
          `Unexpected token found while parsing task declaration. Expected identifier.`
        );
      }
      return arg.symbol;
    });
  ...
  parse_var_declaration() {
    ...
    function parse_variable() {
      ...
      if (constant && !value) {
        throw new Error("Expected expression or value following 'always'.");
      }
      variables.push({
        identifier: {
          kind: "Identifier",
  ...
  parse_args() {
    ...
    const args =
      this.at().type == 23 /* CloseParen */ ? [] : this.parse_args_list();
    this.expect(
      23 /* CloseParen */,
  ...
  if (index.type == "number") {
    ...
    if (caller.type == "list") {
      ...
      if (numberIndex.value < 0) {
        ...
        result =
          listCaller.values[listCaller.values.length + numberIndex.value];
      } else {
...
function eval_logical_expr(node, env, io) {
  ...
  if (lhs.type == "boolean") {
    ...
    if (node.operator == "and") {
      ...
      if (lhs.value) return MK_BOOL(true);
      const rhs = evaluate(node.right, env, io);
      result = rhs.value;
    } else {
...
function eval_if_statement(statement, env, io) {
  ...
  if (condition.type != "boolean") {
    throw new Error(`Invalid if condition, expected boolean`);
  }
  let lastEvaluated = MK_NULL();
...
function eval_for_from_statement(statement, env, io) {
  ...
  if (from.type != "number" || to.type != "number") {
    throw new Error("Invalid for-from statement, expected numbers");
  }
  const evaluations = [];
  const scope = new Environment(env);
...
function eval_for_in_statement(statement, env, io) {
  ...
  if (iterable.type != "list") {
    throw new Error("Invalid for-in statement, expected list");
  }
  const evaluations = [];
  const scope = new Environment(env);
  scope.declareVar(statement.identifier.symbol, MK_NULL(), false);
...
function eval_from_statement(statement, env, io) {
  ...
  if (from.type != "number" || to.type != "number" || by.type != "number") {
    throw new Error("Invalid from statement, expected numbers");
  }
  const evaluations = [];
  const scope = new Environment(env);
  if (from.value < to.value) {
...
function evaluate(astNode, env, io) {
  switch (astNode.kind) {
    ...
    default:
      throw new Error(
        `This ast-node has not yet been implemented for evaluation: ${JSON.stringify(
          astNode
        )}`
      );
  }
